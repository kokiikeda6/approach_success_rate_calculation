...
count, bins, ignored = \
  plt.hist(df_smpl[label_e], 50, density=True, align="mid",
          # label=f"error distribution(μ={mu}, σ={sigma})",
...
plt.vlines(na, ymin, ymax, colors="gray", linestyles="dashed")#, label="left: "+str(na)+"[mm]")

plt.vlines(b, ymin, ymax, colors="red", linestyles="dashed")#, label="right: "+str(b)+"[mm]")
plt.vlines(nb, ymin, ymax, colors="gray", linestyles="dashed")#, label="right: "+str(nb)+"[mm]")

plt.title(f"success rate = {success_rate:.2f}")
plt.xlabel("error [mm]")                # x軸のラベル
